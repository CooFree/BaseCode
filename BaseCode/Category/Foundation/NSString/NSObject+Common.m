...
+ (void)changeBaseURLStrToTest:(NSInteger)isTest{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
...
//    }
    if ([self createDirInCache:kPath_ResponseCache]) {
        NSString *abslutePath = [NSString stringWithFormat:@"%@/%@.plist", [self pathInCacheDirectory:kPath_ResponseCache], [requestPath stringWithMD5]];
